<template>
   <div>
        ...
        <div class="time-entries">
            <p v-if="plans.length==0"><strong>还没有任何计划</strong></p>
...
<script>
     export default {
        ...
        mounted() {
            this.$store.state.list=JSON.parse(localStorage.getItem('vuePlanList')|| '[]')
        },
        computed : {
          plans () {
            // 从store中取出数据
        ...
        updated(){
			   localStorage.setItem('vuePlantime', JSON.stringify( this.$store.state.totalTime))
		}
    }
